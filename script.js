...
function _renderSkillsGrid() {
  ...
  c.innerHTML = ST.skills.map(sk => {
    ...
    <div class="skill-card">
      ...
      <div class="skc-acts" style="flex-wrap: wrap;">
        ...
        <button class="btn-sm-out" style="color:var(--primary); width: 100%; border-color:var(--primary);" onclick="generatePath(${sk.id}, '${_esc(sk.name)}')">✨ Generate Learning Path</button>
      </div>
    </div>`;
  }).join('');
}
...
function onLogSkillChange() {
  const skillId = parseInt(document.getElementById('log-skill').value);
  const datalist = document.getElementById('log-topics');
  if (!datalist) return;
  datalist.innerHTML = '';

  if (!skillId) return;
  const sk = ST.skills.find(s => s.id === skillId);
  if (sk && Array.isArray(sk.session_names)) {
    sk.session_names.forEach(topic => {
      if (topic) {
...
function quickLog(skillId) {
  goPage('log');
  setTimeout(() => {
    const d = document.getElementById('log-skill');
    if (d) {
      d.value = skillId;
      onLogSkillChange();
    }
...
let currentAiPath = [];
let currentAiSkillId = null;

async function generatePath(skillId, skillName) {
  currentAiSkillId = skillId;
  currentAiPath = [];

  // Show Modal
  document.getElementById('ai-skill-name').textContent = skillName;
  document.getElementById('ai-results').innerHTML = '';
  document.getElementById('ai-save-btn').style.display = 'none';
  document.getElementById('ai-loading').style.display = 'block';
  document.getElementById('ai-modal').style.display = 'flex';

  // Use the GEMINI_API_KEY defined globally in supabase-config.js

  const prompt = `You are an expert learning coach. Create a structured learning path for '${skillName}'. 
Return exactly 8 sessions or more if it nedded add session according to the skill requires as a JSON array like this:
[{ "session": 1, "title": "...", "description": "...(one line)" }]
No extra text. Only raw JSON.`;
  ...
  try {
    ...
    try {
      currentAiPath = JSON.parse(text);
    } catch (parseErr) {
      console.error('Failed to parse JSON. Raw response:', text);
      throw new Error('AI returned invalid format.');
    }

    if (!Array.isArray(currentAiPath) || currentAiPath.length === 0) {
    ...
    showToast(err.message || 'Could not generate path. Try again.', 'error');
  } finally {
...
function renderAiPath() {
  ...
  container.innerHTML = currentAiPath.map(s => `
    <div class="ai-session-card">
      <div class="ai-session-badge">${s.session}</div>
...
async function saveAiPath() {
  if (!currentAiPath.length || !currentAiSkillId) return;
  const btn = document.getElementById('ai-save-btn');
  btn.disabled = true; btn.textContent = 'Saving...';

  const titles = currentAiPath.map(s => s.title);

  try {
    const { data, error } = await sb.from('skills').update({
      session_names: titles,
      goal: titles.length,
      updated_at: new Date().toISOString()
    }).eq('id', currentAiSkillId).eq('user_id', ST.user.id).select().single();
    ...
    showToast('Learning path saved to sessions!', 'success');
    closeAiModal();
    refreshDashboard();
  } catch (err) {
    showToast(err.message, 'error');
  } finally {
    btn.disabled = false; btn.textContent = 'Save to My Sessions';
  }
}
...
function downloadWeeklyReport() {
  ...
  const recentSessions = ST.progress.filter(p => new Date(p.session_date) >= sevenDaysAgo);

  if (recentSessions.length === 0) {
    showToast("No sessions logged this week yet!", "error");
    return;
  }
  ...
  const uniqueSkills = new Set(recentSessions.map(p => p.skill_id)).size;

  // Simple streak calculation (consecutive days ending recently)
  let streak = 0;
  let checkDate = new Date();
  checkDate.setHours(0, 0, 0, 0);

  // Get all dates (midnight)
  const allDates = [...new Set(ST.progress.map(p => {
  ...
  let todayMs = checkDate.getTime();
  let yesterdayMs = todayMs - 86400000;

  if (allDates.includes(todayMs) || allDates.includes(yesterdayMs)) {
    let currentMs = allDates.includes(todayMs) ? todayMs : yesterdayMs;
    while (allDates.includes(currentMs)) {
      streak++;
      currentMs -= 86400000;
    }
  }

  const startDateStr = sevenDaysAgo.toLocaleDateString();
  const endDateStr = now.toLocaleDateString();

  // 1. Header
  doc.setFontSize(22);
  doc.setFont("helvetica", "bold");
  doc.text("SkillTracker - Weekly Report", 20, 20);

  // 2. Subheader
  doc.setFontSize(12);
  doc.setFont("helvetica", "normal");
  doc.text(`Week of ${startDateStr} to ${endDateStr}`, 20, 30);
  ...
  // 4. Stats block
  doc.setFont("helvetica", "bold");
  doc.text("Total Sessions This Week:", 20, 45);
  doc.setFont("helvetica", "normal");
  doc.text(`${totalSessions}`, 80, 45);

  doc.setFont("helvetica", "bold");
  doc.text("Skills Practiced:", 20, 52);
  doc.setFont("helvetica", "normal");
  doc.text(`${uniqueSkills}`, 80, 52);

  doc.setFont("helvetica", "bold");
  doc.text("Current Streak:", 20, 59);
  doc.setFont("helvetica", "normal");
  doc.text(`${streak} days`, 80, 59);

  // 5. Divider
  doc.line(20, 65, 190, 65);

  // 6. Sessions Breakdown
  doc.setFontSize(14);
  doc.setFont("helvetica", "bold");
  doc.text("Sessions Breakdown", 20, 75);
  ...
  doc.setFont("helvetica", "bold");
  doc.text("Skill", 22, startY);
  doc.text("Topic", 80, startY);
  ...
  recentSessions.reverse().forEach((session, index) => {
    ...
    const skillName = session.skills?.name || ST.skills.find(s => s.id === session.skill_id)?.name || 'Unknown';
    const topic = session.session_title || 'General Practice';
    const d = new Date(session.session_date).toLocaleDateString();

    // Truncate text if needed
    const safeSkill = doc.splitTextToSize(skillName, 55)[0];
    const safeTopic = doc.splitTextToSize(topic, 75)[0];

    doc.setFont("helvetica", "bold");
    doc.text(safeSkill, 22, startY);

    doc.setFont("helvetica", "normal");
    doc.text(safeTopic, 80, startY);
    doc.text(d, 160, startY);
  ...
  doc.text("Keep going. Consistency beats perfection.", 20, startY);

  startY += 6;
  doc.setFontSize(9);
  doc.setFont("helvetica", "normal");
  doc.setTextColor(150, 150, 150);
  doc.text(`Generated by SkillTracker on ${now.toLocaleDateString()}`, 20, startY);

  // Save PDF
  const filename = `SkillTracker_WeeklyReport_${now.toISOString().split('T')[0]}.pdf`;
  doc.save(filename);
}
